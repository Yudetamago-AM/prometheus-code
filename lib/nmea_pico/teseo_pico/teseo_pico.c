...
int edit_sentence (char sentence[]){
    ...
    for(i = 0;i < length;i ++){
        ...
        switch(str){
            ...
            //printf("checksum_cal:%ld\n", checksum_cal);
            //printf("checksum_true_int:%ld\n", checksum_true_int);
            //printf("checksum_true:%s\n", checksum_true);
            //checksum comparison
    ...
    //return checksum_cal for PSTMLOWPOWERONOFF
    if(after_checksum = false) return checksum_cal;
}
...
int main() {
    ...
    while (1){
        int i;
        char buf[] = "$PSTMLOWPOWERONOFF,0,0*";
        char str[(strlen(buf) + 5)];
        int check = edit_sentence(buf);
        snprintf(str, (strlen(buf) + 5), "%.*s%02X\n", (int)strlen(buf), buf, check);
        for(i = 0;i < 10;i ++){
            printf("%s", str);
        ...
        //output
        if(ready && rmc_ready){
            //printf("小数点ガン無視。\n緯度(latitude)は最大位から3つ目以下が小数。\n経度(longitude)は最大位から4つ目以下が小数。\n");
